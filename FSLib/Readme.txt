...
SNAP SDK, so those folders are also reachable. The makefile also uses the
'implib' tool from the OS/2 Developer's Toolkit, so make sure that this file
is on the path, or it's copied here before compiling the DLL!

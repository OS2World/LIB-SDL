...
#ifdef __OS2__
// We'll increase the priority of GobbleEvents thread, so it will process events in time for sure!
// For this, we need the DosSetPriority() API frfrom the os2.h include file.
...
static int SDL_GobbleEvents(void *unused)
{
        ...
        // Increase the priority of this thread, so it will process events in time for sure!
